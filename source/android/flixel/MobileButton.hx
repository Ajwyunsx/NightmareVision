...
class TypedMobileButton<T:FlxSprite> extends FlxSprite implements IFlxInput
{
	...
	**/
	...
	function updateButton():Void
	{
		...
		if (status != MobileButton.NORMAL && (!overlapFound || (currentInput != null && currentInput.justReleased))) onOutHandler();
	}
	...
	function set_statusIndicatorType(Value:StatusIndicators)
	{
		...
		{
			shader = brightShader;
			if (_spriteLabel != null) _spriteLabel.shader = brightShader;
		}
		else
		{
			shader = null;
			if (_spriteLabel != null) _spriteLabel.shader = null;
		}
