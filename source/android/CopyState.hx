...
class CopyState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		if (shouldCopy && copyLoop != null)
		{
			...
			if (loopTimes == maxLoopTimes) loadedText.text = "Completed!";
			else loadedText.text = '$loopTimes/$maxLoopTimes';
		}
	...
	public function copyAsset()
	{
		...
		if (!FileSystem.exists(file))
		{
			...
			try
			{
				...
				{
					if (textFilesExtensions.contains(Path.extension(file))) createContentFromInternal(file);
					else File.saveBytes(file, getFileBytes(getFile(file)));
				}
				else
				{
	...
	public static function checkExistingFiles():Bool
	{
		...
		// Exclude androidcontrols, touchpad and embeds folders (requested by user)
		locatedFiles = locatedFiles.filter(file ->
			file.indexOf('assets/images/androidcontrols') == -1 &&
			file.indexOf('assets/images/touchpad') == -1 &&
			file.indexOf('assets/embeds') == -1
		);
		...
		{
			if (filesToRemove.contains(file)) continue;
			
			if (file.endsWith(IGNORE_FOLDER_FILE_NAME) && !directoriesToIgnore.contains(Path.directory(file))) directoriesToIgnore.push(Path.directory(file));
			
			if (directoriesToIgnore.length > 0)
			{
				for (directory in directoriesToIgnore)
				{
					if (file.startsWith(directory)) filesToRemove.push(file);
				}
			}
		}
